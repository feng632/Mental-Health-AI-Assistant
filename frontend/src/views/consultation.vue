                    ...
                    <div class="session-info">
                        <div class="session-title">
                            ...
                            <div class="session-stats">
                                ...
                                <span class="user-count">{{session.userCount}}</span>
                            </div>
                        </div>
        ...
        <div class="chat-main">
            ...
            <div class="chat-messages">
                <div class="message-item ai-message" v-if="messages.length === 0">
                    ...
                    <div class="message-content">
                        <div class="message-bubble">
                            <p>您好，我是宁渡AI助手，您的心理健康助手。很高兴为您提供专业的心理健康服务。请告诉我您的问题或需求，我会尽力帮助您。</p>
                        </div>
                ...
                <div v-for="msg in messages" :key="msg.id" class="message-item " :class="msg.senderType === 1 ? 'user-message':'ai-message'">
                    ...
                    <div class="message-content">
                        ...
                        <div class="message-time">{{msg.senderType === 2 && isAiTyping.value ? '正在思考中...' : msg.createdAt}}</div>
                    </div>
                </div>
            </div>
            <div class="chat-input">
                <div class="input-container">
                    ...
                    :rows="3"
                    :disabled="isAiTyping"
                    @keydown="handleKeyDowm"
                    class="message-input"
                    clearable
                    />
                    <div class="input-footer">
                        <span>按住Enter键发送，按住Enter+Shift换行</span>
                        <span>{{userMessage.length}}/500</span>
                    </div>

                </div>
                <el-button :disabled="isAiTyping || !userMessage.trim() || userMessage.length > 500" class="send-btn" type="primary" @click="sendMessage">
                    <el-icon>
                        <Promotion />
                    </el-icon>
                </el-button>
...
const loadSessionEmotion =  (sessionId) => { 
    //确保sessionId格式正确
    const id = sessionId.toString().startsWith('session_')?sessionId:sessionId = `session_${sessionId}`
...
const sendMessage = () => {
    ...
    if (currentSession.value.status === 'TEMP') {
        ...
        starAiResponse(currentSession.value.sessionId, message)
    }
}
...
const starAiResponse = (sessionId, message) => { 
    if(isAiTyping.value) {
        ElMessage.warning('AI正在思考中，请稍后')
        return
    }
    ...
    fetchEventSource('/api/psychological-chat/stream', {
       ...
       signal: ctrl.signal,
       onopen: (response) => { 
           // Content-Type 是 text/event-stream 说明连接正常建立
       ...
       onmessage: (event) => { 
           ...
           // 调试日志：看结尾到底发了什么
           console.log('sse event:', event.event, raw)
           if(!raw) {
               return
           }
           const aiMessage = messages.value[messages.value.length - 1]

           // 结束事件：正常收尾，不再按内容/错误解析
           if(event.event === 'done') {
           ...
           try {
               ...
               // 结尾若带非 JSON 的终止标记，忽略而不是报错
               console.log('sse 非JSON数据:', raw)
               return
           }
